...
try:
    ...
    config = get_config()
    TAXONOMY_PATH = getattr(config, 'PROBLEM_DB_PATH', 'problem_codes.json')
except ImportError:
    config = None
    # Auto-resolve: prefer root-level, fallback to data/
    TAXONOMY_PATH = 'problem_codes.json' if Path('problem_codes.json').exists() else 'data/problem_codes.json'
...
try:
    ...
    from H2L_core import H2LConfigV3, compare_v2_v3
        ...
        if config and _HAS_H2L_COMPONENTS:
            try:
                self.orchestrator = RAGFirstOrchestrator(
                    ...
                    problem_codes_path=taxonomy_path or "problem_codes.json",
                    enable_l2=enable_l2
                )
                            ...
                            with gr.Tab("📊 ประเมิน Ground Truth"):
                                with gr.Row():
                                    with gr.Column(scale=1):
                                        ...
                                        if not gt_files:
                                            gt_files = ["expanded_ground_truth.json"]

                                        eval_gt_selector = gr.Dropdown(
                                            choices=gt_files,
                                            value="expanded_ground_truth.json",
                                ...
                                def _run_neg_eval(gt_file, model_type):
                                    ...
                                    try:
                                        ...
                                        results = evaluate_sentence_polarity(gt_file, "problem_codes.json", model_type)
                            ...
                            with gr.Tab("🔍 วิเคราะห์เคสเดี่ยว"):
                                ...
                                def _analyze_single_case(text, model_type):
                                    ...
                                    try:
                                        ...
                                        taxonomy_path = _Path(__file__).parent / "problem_codes.json"
                        ...
                        def _run_sensitivity(max_cases):
                            ...
                            try:
                                ...
                                analyzer = SensitivityAnalyzer(
                                    ground_truth_path="expanded_ground_truth.json",
