...
# เปิด/ปิดข้อความละเอียดมาก (True จะแสดง retry, timeout, split ย่อย)
VERBOSE = True

# ---------- Heartbeat ----------
class Heartbeat:
    """
    แสดงสัญญาณว่ากำลังทำงานอยู่ระหว่างรอ network I/O (เช่น requests.post ที่บล็อก)
...
# ---------- Core OCR (with retry/backoff + heartbeat) ----------
def extract_text_from_image(
    ...
    while attempt <= retries:
        hb = Heartbeat(
            hb_label or f"POST Typhoon pages {pages[0]}-{pages[-1]} (attempt {attempt+1}/{retries})",
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                if VERBOSE:
                    tqdm.write(f"→ POST {Path(file_path).name} pages {pages[0]}-{pages[-1]} (len={len(pages)}), attempt {attempt+1}/{retries}")
            ...
            tqdm.write(f"⏱️ Timeout → รอ {wait:.1f}s แล้วลองใหม่ (attempt {attempt}/{retries})")
            time.sleep(wait)
            continue
            ...
            if attempt > retries:
                ...
                return None
            wait = backoff ** attempt
            tqdm.write(f"🌐 Network error: {e} → รอ {wait:.1f}s แล้วลองใหม่ (attempt {attempt}/{retries})")
            time.sleep(wait)
            continue
        finally:
            # หยุด heartbeat เมื่อจบคำขอ
            try:
                hb.stop()
            except Exception:
                pass
        ...
        if resp.status_code in (429, 500, 502, 503, 504):
            ...
            if attempt > retries:
                ...
                return None
            wait = backoff ** attempt
            tqdm.write(f"📶 API {resp.status_code} → รอ {wait:.1f}s แล้วลองใหม่ (attempt {attempt}/{retries})")
            time.sleep(wait)
            continue
...
# ---------- OCR สำหรับไฟล์เดียว (tqdm + probe + adaptive chunk) ----------
...
if __name__ == "__main__":
    ...
    if not pdf_files:
        print(f"⚠️ ไม่พบไฟล์ PDF ใน {Path(docs_dir).resolve()}")
    else:
        print(f"🔍 พบไฟล์ PDF {len(pdf_files)} ไฟล์ใน {Path(docs_dir).resolve()}")
