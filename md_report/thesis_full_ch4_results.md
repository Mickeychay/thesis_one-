...
บทนี้นำเสนอผลการทดลองและการวิเคราะห์ข้อมูลจากการประเมินประสิทธิภาพของระบบ H2L โดยแบ่งการรายงานออกเป็น 6 ส่วนหลัก ได้แก่ (1) ข้อมูลชุดทดสอบที่ผ่านการตรวจสอบความถูกต้อง (2) ผลการประเมินการตรวจจับความขัดแย้ง (Sentence Polarity) (3) ผลการประเมินประสิทธิภาพการค้นคืนเอกสาร (Retrieval Performance) (4) ผลการทดสอบบทบาทขององค์ประกอบย่อย (Ablation Study) (5) การวิเคราะห์ความไวของพารามิเตอร์ (Parameter Sensitivity Analysis) และ (6) สถานะของการประเมินโดยผู้เชี่ยวชาญแบบ blind
...
เนื่องจากระบบ H2L ใช้พารามิเตอร์ที่กำหนดจากความรู้เชิงโดเมน (expert-elicited) มากกว่าการ tune ด้วยข้อมูล ผู้วิจัยจึงทำ **One-at-a-Time (OAT) sensitivity analysis** เพื่อตรวจสอบว่าผลของระบบเสถียรเพียงใดเมื่อพารามิเตอร์ถูกเปลี่ยนไปจากค่าเริ่มต้น โดยปรับแต่ละพารามิเตอร์ทีละตัว ±30-50% จากค่าเริ่มต้น และวัดผลกระทบต่อค่า `mean_score` บนเคสจำนวน 122 เคสจาก ground truth ที่ครอบคลุมทั้งเคสฉุกเฉิน เคสทั่วไป และเคสที่มี polarity ซับซ้อน รายละเอียดเต็มและภาพประกอบ (heatmap, tornado plot) อยู่ใน `sensitivity_results/`

**ตารางที่ 4.6 ผลการวิเคราะห์ความไวของพารามิเตอร์หลัก (n=122 cases, baseline mean_score = 1.4466)**

| พารามิเตอร์ | Default | Range ที่ทดสอบ | Max \|Δ\|% | Verdict |
|:---|---:|:---:|---:|:---|
| $\alpha_0$ (Base Weight) | 1.0 | [0.25, 2.0] | **219.68%** | ❌ **Sensitive** |
| $T_{\text{base}}$ (Calibration) | 0.5 | [0.2, 1.0] | **17.51%** | ⚠️ **Sensitive** |
| $T_{\text{range}}$ (Calibration) | 1.5 | [0.5, 2.5] | 2.25% | ✅ Robust |
| $\mu$ (Dirichlet) | 2.0 | [0.5, 4.0] | 0.24% | ✅ Robust |
| $\kappa$ (KL Penalty) | 0.15 | [0.0, 0.3] | 0.17% | ✅ Robust |
| $\lambda_{\text{neg}}$ (Polarity Gate) | 0.6 | [0.3, 1.0] | 0.00% | ✅ Robust |
| $m$ (Margin) | 0.3 | [0.1, 0.5] | 0.00% | ✅ Robust |
| $\beta$ (L1/L2 Balance) | 0.3 | [0.0, 1.0] | 0.00% | ✅ Robust |

**การวิเคราะห์ผล (Findings)**:

1. **6 ใน 8 พารามิเตอร์เสถียร** ($|\Delta| < 5\%$): พารามิเตอร์ที่ผู้วิจัยเลือกจากความรู้เชิงโดเมน ได้แก่ $\lambda_{\text{neg}}$, $\kappa$, $m$, $\mu$, $\beta$ และ $T_{\text{range}}$ ให้ผลแทบไม่เปลี่ยนเมื่อปรับ ±30-50% สนับสนุนการเลือกใช้ค่าเหล่านี้ตามทฤษฎี (เช่น $\lambda_{\text{neg}}=0.6$ จาก rule-based negation literature, $m=0.3$ จาก margin-based discrimination) โดยไม่ต้อง grid-search

2. **$\alpha_0$ (Base Weight) มีความอ่อนไหวสูงมาก** (Δ +219.68% ที่ค่าสูงสุดของช่วงทดสอบ): พารามิเตอร์นี้เป็นตัวคูณ scaling สำหรับทั้ง weighted-sum ดังนั้นการเพิ่มค่าจะขยายคะแนนทั้งระบบเป็นสัดส่วน ความอ่อนไหวจึงเป็นผลทางคณิตศาสตร์ของการเป็น multiplicative scaling factor ไม่ใช่ความไม่เสถียรเชิง model ทั้งนี้ ค่าเริ่มต้นของระบบ ($\alpha_0=1.0$) ตั้งให้เป็นค่ากลาง (no scaling) เพื่อเป็น reference point ในการตีความคะแนนสัมพัทธ์ระหว่างเคส

3. **$T_{\text{base}}$ (Calibration Temperature) มีความอ่อนไหวระดับปานกลาง** ($|\Delta|=17.51\%$): พารามิเตอร์นี้ควบคุม sharpness ของ severity-weighted confidence calibration การเปลี่ยน $T_{\text{base}}$ จะเปลี่ยน contrast ระหว่างคะแนนของเคสรุนแรงและเคสไม่รุนแรง ผู้วิจัยจึงเลือก $T_{\text{base}}=0.5$ (กึ่งกลางของช่วง 0.2-1.0) เพื่อให้ระบบไม่ over-confident กับเคสรุนแรงและไม่ under-confident กับเคสทั่วไป ซึ่งสอดคล้องกับการตีความเชิงคลินิกที่ต้องไม่ตื่นตระหนกเกินสัญญาณและไม่มองข้ามสัญญาณเตือนภัย

**ข้อจำกัดและการตีความ**: การที่ $\alpha_0$ และ $T_{\text{base}}$ มีความอ่อนไหวสูงไม่ได้บั่นทอนความถูกต้องของผลในตารางที่ 4.3-4.5 เนื่องจากในการประเมินผลทั้งหมด ระบบใช้ค่าเริ่มต้นชุดเดียวกัน (`α_0=1.0`, `T_base=0.5`) ซึ่งเป็นค่ามาตรฐานที่กำหนดล่วงหน้าก่อนเริ่มประเมิน ไม่ใช่ค่าที่ปรับ post-hoc ให้ผลออกมาดี อย่างไรก็ตาม ในการ deploy ระบบในบริบทใหม่หรือชุดข้อมูลใหม่ การ recalibrate $T_{\text{base}}$ ตามการกระจายตัวของ severity ในชุดข้อมูลนั้น ๆ เป็นขั้นตอนที่ผู้วิจัยแนะนำให้ทำเป็น pre-deployment check

---

## 4.6 Blind Expert Evaluation
