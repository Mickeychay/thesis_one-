...
ผลนี้บ่งชี้ว่า **คุณค่าหลักของ H2L** อยู่ที่การกรอง candidate document ผ่าน problem-aware scoring (เทียบกับ baseline ที่ไม่มี H2L ซึ่งต่ำกว่าอย่างมีนัยสำคัญตามตารางที่ 4.4) ไม่ใช่ความแตกต่างระหว่าง V6 sub-components ในชุดทดสอบนี้ การทดสอบเพิ่มเติมที่ออกแบบไว้เพื่อเปรียบเทียบ severity-weighted Bayesian prior กับ uniform prior ยืนยันข้อค้นพบนี้: ทั้งสองเงื่อนไขให้ผลเหมือนกันทุกตัวชี้วัดทุกกรณีศึกษา (ค่าต่างสัมบูรณ์สูงสุด $= 0.000$) เนื่องจาก `w/o Bayesian Prior` ในตารางที่ 4.6 ให้ nDCG@5 $= 0.2442$ เท่ากับ Full V6 ทุกกรณีศึกษา จึงยังไม่มีหลักฐานเชิงประจักษ์ว่า severity-weighted prior ส่งผลต่ออันดับเอกสารบนชุดทดสอบนี้
