...
**Generated**: 2026-07-30 10:04

**Baseline Score**: 1.484015
**Baseline Boost**: 3.2978
**Cases Evaluated**: 129

### 📌 แหล่งที่มาและประเภทของเคสทดสอบ (Test Cases Context)
เคสจำนวนทั้งหมด **129** เคส ถูกดึงมาจากไฟล์ Ground Truth ซึ่งสร้างขึ้นจากฐานข้อมูลปัญหาผู้ใช้งานจริง (Real-world user queries) และได้รับการเฉลย (Label/Annotate) โดยอิงตามแนวทางการประเมินผู้รับบริการจริง
...
|-----------|---------|--------|--------|---------|---------|
| T_base (Calibration) | 0.5 | -17.86% | +11.63% | 17.86% | ❌ Sensitive (อ่อนไหว) |
| T_range (Calibration) | 1.5 | -2.21% | +1.29% | 2.21% | ✅ Robust (เสถียร) |
| λ_neg (Polarity Gate) | 0.6 | +0.00% | +0.00% | 0.00% | ✅ Robust (เสถียร) |
| κ (KL Penalty) | 0.15 | -0.16% | +0.16% | 0.16% | ✅ Robust (เสถียร) |
| m (Margin) | 0.3 | +0.00% | +0.00% | 0.00% | ✅ Robust (เสถียร) |
| μ (Dirichlet) | 2.0 | -0.22% | +0.16% | 0.22% | ✅ Robust (เสถียร) |
| α₀ (Base Weight) | 1.0 | -59.53% | +226.04% | 226.04% | ❌ Sensitive (อ่อนไหว) |
| β (L1/L2 Balance) | 0.3 | +0.00% | +0.00% | 0.00% | ✅ Robust (เสถียร) |

## สรุปภาพรวมความเสถียร (Overall Verdict)

> ❌ **มีความอ่อนไหวสูง (SENSITIVE)**: โมเดลผันผวนสูงมากเมื่อเปลี่ยนค่าตัวแปรหลัก (คะแนนเหวี่ยงสูงสุด 226.0%) ควรมีการตรวจสอบการตั้งค่าตัวแปรนี้เพิ่มเติม